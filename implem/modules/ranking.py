...
class AttributesRanking():
    ...
    def __init__(self, attributes, star):
        ...
        self.pairwise_qualities_2 = []

        for enum,k in enumerate([['good','less_good'],['good','bad'],['less_good','bad'],['good','very_bad'],['less_good','very_bad'],['bad','very_bad']]):
            
            liste = list(itertools.permutations(k))
            
            if enum == 0:
                self.pairwise_qualities_2 += [('good', 'good')] + liste + [('less_good', 'less_good')]
            elif enum < 5:
                self.pairwise_qualities_2 += liste
            else:
                self.pairwise_qualities_2 += [('bad', 'bad')] + liste + [('very_bad', 'very_bad')]
    ...
    def calculate_ranking(self, gb_or_agg, panel, diversity, forbidden_attributes=list()):
        ...
        if gb_or_agg == 'aggregation':
            ...
            if len(forbidden_list) == len(self.preprocessed['groupBy']):
                # in case all attributes are forbidden allow forbidden except already_on_same
                forbidden_list = list(panel.aggregates.keys())        
            ...
            if len(forbidden_list) == len(self.preprocessed['groupBy']):    # in case all attributes are forbidden, allow forbidden except already_on_same
                forbidden_list = panel.groupBy  
